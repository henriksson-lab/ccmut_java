...
def color_oneseq(oneseq,origseq,repseq):
    ...
    colors_after = colors_for_seq[1:len(colors_for_seq)]
    colors_red = [redcolor(x[0],x[1]) for x in zip(colors_before, colors_after)]

    #Color the sequence
    colored_seq = ''.join([color_char(x[0],x[1]) for x in zip(colors_red,list(oneseq))])
    return '<p style=\"color:black;\">' + colored_seq + '</p>'
